...
def get_schedule(device_id):
    last_weeks_day_start = datetime.combine(datetime.today(), time.min) - timedelta(days=7)
    last_weeks_day_end = datetime.combine(datetime.today(), time.max) - timedelta(days=7)
    last_weeks_day_events = Event.objects.filter(timestamp__gte=last_weeks_day_start).filter(timestamp__lte=last_weeks_day_end)  
    ...
    yesterday_events = Event.objects.filter(timestamp__gte=yesterday_start).filter(timestamp__lte=yesterday_end)  
